...
import { CurrentLangFunc, DefaultSupportedLangsArray } from './multilang';
...
export class MultiLangModule {
    ...
    static forRoot<T extends string>(config: MultiLangModuleConfig<T>): ModuleWithProviders<MultiLangModule> {

        config = {
            supportedLangs: config?.supportedLangs || [...DefaultSupportedLangsArray] as T[],
            currentLang: {
                inject: config?.currentLang?.inject || [],
                factory: config?.currentLang?.factory || (() => () => DefaultSupportedLangsArray[0] as T)
            }
        }
